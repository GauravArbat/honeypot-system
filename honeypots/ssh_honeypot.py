...
class SSHHoneypot:
    ...
    def log_attack(self, client_ip, username, password):
        """Send attack log to backend API and check for alerts"""
        try:
            data = {
                'source_ip': client_ip,
                'service': 'SSH',
    ...
    def check_alerts(self, client_ip):
        """Check if IP should trigger alert"""
        try:
            conn = sqlite3.connect('honeypot.db')
            count = conn.execute('SELECT COUNT(*) FROM attack_logs WHERE source_ip = ?', (client_ip,)).fetchone()[0]
            conn.close()
            
            # Trigger alert if threshold exceeded
            alert_manager.check_and_alert(client_ip, count, "SSH")
    ...
    def handle_client(self, client_socket, client_address):
        ...
        try:
            ...
            # Log the attack and check alerts
